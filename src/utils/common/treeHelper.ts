 ...
 */
export interface TreeHelperConfig<T = any> {
	idKey: keyof T;
	childrenKey: keyof T;
	parentKey: keyof T;
}
...
export function flattenToTree<T>(list: T[], config: Partial<TreeHelperConfig> = {}): T[] {
	...
	for (const node of list) {
		const childrenNodes: T[] = (node as any)[childrenKey] || [];
		nodeMap.set((node as any)[idKey], { ...node, [childrenKey]: childrenNodes });
	}
		...
		return nodes
			...
			.filter((node) => {
				(node as any)[childrenKey] =
					(node as any)[childrenKey] && filterSubtree((node as any)[childrenKey]);
				return predicate(node) || ((node as any)[childrenKey] && (node as any)[childrenKey].length);
			});
	}
...
export function mapTreeStructure<T = any, R = any>(
	...
	return treeData.map((node) =>
		mapTreeNodeStructure(node, convertNodeToStructure, getTreeHelperConfig(config))
	);
}
...
export function mapTreeNodeStructure<T = any, R = any>(
	...
	const { childrenKey } = getTreeHelperConfig(config);
	const hasChildren = isArray((node as any)[childrenKey]) && (node as any)[childrenKey].length > 0;
	const convertedNode = convertNodeToStructure(node) || {};
	if (hasChildren) {
		return {
			...convertedNode,
			[childrenKey]: (node as any)[childrenKey].map((childNode: T) =>
				mapTreeNodeStructure(childNode, convertNodeToStructure, config)
			)
		};
	}
	return {
